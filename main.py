...
if __name__ == '__main__':
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        acc_history.append(avg_loss[0])
        classifier.eval()
        test_loss = loop_dataset(test_graphs, classifier, list(range(len(test_graphs))))
        if not cmd_args.printAUC:
            test_loss[2] = 0.0
        print('\033[93maverage test of epoch %d: loss %.5f acc %.5f auc %.5f\033[0m' % (epoch, test_loss[0], test_loss[1], test_loss[2]))
        val_acc_history.append(test_loss[0])
    ...
    # Create folder where to store the model
    if not os.path.exists('./Saved_Models'):
        os.mkdir('./Saved_Models')
    # Store the model
    print("[INFO] serializing network to '{}'...".format(cmd_args.data))
    torch.save(classifier.state_dict(), './Saved_Models/MyModel.pt')
